...
namespace Ceilapp
{
    public partial class ceilappService
    {
        ...
        public async Task ExportCompensationsToExcel(Query query = null, string fileName = null)
        {
            navigationManager.NavigateTo(query != null ? query.ToUrl($"export/ceilapp/compensations/excel(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')") : $"export/ceilapp/compensations/excel(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')", true);
        }

        public async Task ExportCompensationsToCSV(Query query = null, string fileName = null)
        {
            navigationManager.NavigateTo(query != null ? query.ToUrl($"export/ceilapp/compensations/csv(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')") : $"export/ceilapp/compensations/csv(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')", true);
        }

        partial void OnCompensationsRead(ref IQueryable<Ceilapp.Models.ceilapp.Compensation> items);

        public async Task<IQueryable<Ceilapp.Models.ceilapp.Compensation>> GetCompensations(Query query = null)
        {
            var items = Context.Compensations.AsQueryable();
            ...
            OnCompensationsRead(ref items);

            return await Task.FromResult(items);
        }

        partial void OnCompensationGet(Ceilapp.Models.ceilapp.Compensation item);
        partial void OnGetCompensationById(ref IQueryable<Ceilapp.Models.ceilapp.Compensation> items);


        public async Task<Ceilapp.Models.ceilapp.Compensation> GetCompensationById(int id)
        {
            var items = Context.Compensations
                              .AsNoTracking()
                              .Where(i => i.Id == id);

            items = items.Include(i => i.CourseRegistration);
 
            OnGetCompensationById(ref items);

            var itemToReturn = items.FirstOrDefault();

            OnCompensationGet(itemToReturn);

            return await Task.FromResult(itemToReturn);
        }

        partial void OnCompensationCreated(Ceilapp.Models.ceilapp.Compensation item);
        partial void OnAfterCompensationCreated(Ceilapp.Models.ceilapp.Compensation item);

        public async Task<Ceilapp.Models.ceilapp.Compensation> CreateCompensation(Ceilapp.Models.ceilapp.Compensation compensation)
        {
            OnCompensationCreated(compensation);

            var existingItem = Context.Compensations
                              .Where(i => i.Id == compensation.Id)
                              .FirstOrDefault();
            ...
            {
                Context.Entry(compensation).State = EntityState.Detached;
                throw;
            }

            OnAfterCompensationCreated(compensation);

            return compensation;
        }

        public async Task<Ceilapp.Models.ceilapp.Compensation> CancelCompensationChanges(Ceilapp.Models.ceilapp.Compensation item)
        {
            var entityToCancel = Context.Entry(item);
            if (entityToCancel.State == EntityState.Modified)
            {
        ...
        partial void OnCompensationUpdated(Ceilapp.Models.ceilapp.Compensation item);
        partial void OnAfterCompensationUpdated(Ceilapp.Models.ceilapp.Compensation item);

        public async Task<Ceilapp.Models.ceilapp.Compensation> UpdateCompensation(int id, Ceilapp.Models.ceilapp.Compensation compensation)
        {
            OnCompensationUpdated(compensation);

            var itemToUpdate = Context.Compensations
                              .Where(i => i.Id == compensation.Id)
                              .FirstOrDefault();
            ...
            var entryToUpdate = Context.Entry(itemToUpdate);
            entryToUpdate.CurrentValues.SetValues(compensation);
            entryToUpdate.State = EntityState.Modified;

            Context.SaveChanges();

            OnAfterCompensationUpdated(compensation);

            return compensation;
        }

        partial void OnCompensationDeleted(Ceilapp.Models.ceilapp.Compensation item);
        partial void OnAfterCompensationDeleted(Ceilapp.Models.ceilapp.Compensation item);

        public async Task<Ceilapp.Models.ceilapp.Compensation> DeleteCompensation(int id)
        {
            var itemToDelete = Context.Compensations
                              .Where(i => i.Id == id)
                              .FirstOrDefault();
            ...
            OnCompensationDeleted(itemToDelete);


            Context.Compensations.Remove(itemToDelete);
            ...
            OnAfterCompensationDeleted(itemToDelete);
