        ...
        protected async Task ExportToExcel()
        {
            try
            {
               Ceilapp.Models.ceilapp.CourseRegistration  cr = new Ceilapp.Models.ceilapp.CourseRegistration();
                ...
                // Build filter conditions without repeating 'r =>'
                var filterConditions = new List<string>();
                
                if (SelectedSession.HasValue)
                {
                    filterConditions.Add($"r.SessionId == {SelectedSession.Value}");
                }
        
                if (Validated.HasValue)
                {
                    filterConditions.Add($"r.RegistrationValidated == {Validated.Value.ToString().ToLower()}");
                }

                if (Reinscrition.HasValue)
                {
                    filterConditions.Add($"r.IsReregistration == {Reinscrition.Value.ToString().ToLower()}");
                }
        
                if (SelectedCourse.HasValue)
                {
                    filterConditions.Add($"r.CourseId == {SelectedCourse.Value}");
                }
        
                if (SelectedLevel.HasValue)
                {
                    filterConditions.Add($"r.CourseLevelId == {SelectedLevel.Value}");
                }
                
                // Combine all conditions into a single predicate
                if (filterConditions.Any())
                {
                    query.Filter = $"r => {string.Join(" && ", filterConditions)}";
                }
